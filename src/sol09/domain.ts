...
import { absurd, pipe, flow } from "@effect-ts/core/Function"
...
export class EnvironmentError {
    ...
    constructor(
        ...
        readonly error: MissingVarEnvironmentError | EmptyVarEnvironmentError,
    ) { }
}

export class MissingVarEnvironmentError {
    readonly _tag = "MissingVarEnvironmentError";
    constructor(
        readonly error: unknown
    ) { }
}

export class EmptyVarEnvironmentError {
    _tag = "EmptyVarEnvironmentError";
}

export class InitialPositionError {
    readonly _tag = "InitialPositionError";
}
...
export const turnLeft = (rover: Rover): Rover => {
    switch (rover.orientation) {
        ...
        default:
            return absurd(rover.orientation);
    }
};
...
export interface ColourPalette {
    resetCode: string,
    roverColour: string,
    roverHitColour: string,
    obstacleColour: string,
    terrainColour: string,
    navColour: string,
}

export const emptyPalette: ColourPalette = {
    resetCode: "",
    roverColour: "",
    roverHitColour: "",
    obstacleColour: "",
...
export const renderNav = (outcome: TravelOutcome, palette: ColourPalette = emptyPalette): string => {
    switch (outcome.rover.orientation) {
        case Orientation.N:
            return `  ${palette.navColour}^${palette.resetCode}\n< N >\n  v`;
        case Orientation.E:
            return `  ^\n< E ${palette.navColour}>${palette.resetCode}\n  v`;
        case Orientation.S:
            return `  ^\n< S >\n  ${palette.navColour}v${palette.resetCode}`;
        case Orientation.W:
            return `  ^\n${palette.navColour}<${palette.resetCode} W >\n  v`;
        default:
            return absurd(outcome.rover.orientation);
    }
}
...
export const renderPlanet = (
    ...
    obstacles: ReadonlyArray<Obstacle>,
    leftPad: string = "",
    palette: ColourPalette = emptyPalette,
) => (
    outcome: TravelOutcome,
): string => {
    const objectsMap = pipe(
        obstacles,
        A.reduce(HM.make<Tp.Tuple<[number, number]>, string>(), (dict, obstacle) =>
            HM.set_(
                dict,
                Tp.tuple(obstacle.pos.x, obstacle.pos.y),
                `${palette.obstacleColour}${OBSTACLE_CHAR}${palette.resetCode}`
            )),
        HM.set(
            Tp.tuple(outcome.rover.x, outcome.rover.y),
                `${outcome.kind === "Hit"
                    ? palette.roverHitColour
                    : palette.roverColour}${ROVER_CHAR}${palette.resetCode}`
            ),
    );
    return pipe(
        A.range(0, planet.height - 1),
        A.map((y) => pipe(
            A.range(0, planet.width - 1),
            A.map((x) =>
                O.getOrElseS_(
                   HM.get_(objectsMap, Tp.tuple(x, y)),
                   () => `${palette.terrainColour}${TERRAIN_CHAR}${palette.resetCode}`,
                )
            ),
            A.join(""),
            (str) => `${leftPad}${str}`,
